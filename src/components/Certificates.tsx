...
import { Award, Calendar, ExternalLink, Building, Sparkles, Linkedin, FileText, CheckCircle } from 'lucide-react';
import { portfolioData } from '../data/portfolioData';

export const Certificates: React.FC = () => {
  const { certificates } = portfolioData;

  const getBadgeAndIcon = (url: string) => {
    if (url.includes('lnkd.in') || url.includes('linkedin.com')) {
      return {
        badge: 'Verified on LinkedIn',
        icon: <Linkedin className="w-3.5 h-3.5 text-cyan-400" />
      };
    }
    if (url.includes('.pdf')) {
      return {
    ...
    return {
      ...
      icon: <ExternalLink className="w-3.5 h-3.5 text-cyan-400" />
    };
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <p className="mt-3 text-slate-400 text-sm sm:text-base max-w-2xl">
            Verified course completions and certifications across programming, systems, web, and artificial intelligence.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {certificates.map((cert) => {
            const { badge, icon } = getBadgeAndIcon(cert.certificateUrl);
            return (
              <div
                key={cert.id}
              ...
              >
                ...
                {/* Action Button: Opens Certificate Directly */}
